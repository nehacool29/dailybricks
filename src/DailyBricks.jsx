...
async function saveState(data) {
  try { await localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); } catch {}
}
async function loadState() {
  try {
    const r = await localStorage.getItem(STORAGE_KEY);
    return r ? JSON.parse(r.value) : null;
  } catch { return null; }
}
async function saveAnalytics(data) {
  try { await localStorage.setItem(ANALYTICS_KEY, JSON.stringify(data), true); } catch {}
}
async function loadAnalytics() {
  try {
    const r = await localStorage.getItem(ANALYTICS_KEY, true);
    return r ? JSON.parse(r.value) : null;
  } catch { return null; }
}
